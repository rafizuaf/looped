-- Fix item sale price update issue
-- This migration fixes an issue where the wrong selling price is recorded in the budget
-- when changing an item from unsold to sold while also updating its price

-- Drop the existing function
DROP FUNCTION IF EXISTS update_item_with_transaction;
...
BEGIN
    ...
    BEGIN
        ...
        -- First update the item to ensure the new selling price is used for sales
        UPDATE items
        SET 
            name = p_name,
            category = p_category,
        ...
        -- Update the sold status after handling sale transactions
        UPDATE items
        SET 
